...
function cacheDom() {
  ...
  el.clockDisplay = document.getElementById("clock-display");
  ...
  el.settingsToggle = document.getElementById("settings-toggle");
...
function applyLayout() {
  el.app.classList.toggle("layout-compact", state.config.ui.layout === "compact");
  el.app.classList.toggle("layout-cozy", state.config.ui.layout !== "compact");
...
function renderClockText() {
  const clockText = formatDateTime(new Date(), state.config.widgets.clock.format24h);
  el.clockDisplay.textContent = clockText;
}
...
function handleSearchSubmit(event) {
  ...
  const targetUrl = buildSearchUrl(state.config.search, keyword);
...
function renderQuickLinks() {
  ...
  for (const link of links) {
    ...
    editButton.className = "ghost-btn small-btn";
    ...
    card.append(icon, title, url, actions);
...
function handleCardDragEnd(event) {
  ...
  for (const card of el.quickLinksGrid.querySelectorAll(".quick-link-card")) {
...
function renderTodoWidget() {
  ...
  el.todoWidget.classList.toggle("hidden", !enabled);
  if (!enabled) {
    return;
  }

  el.todoList.innerHTML = "";
  for (const item of state.config.todos) {
    ...
    deleteButton.className = "warn-btn small-btn";
...
function setStatus(message, isError) {
  ...
  el.statusText.style.color = isError ? "var(--warning)" : "var(--text-secondary)";
