import { clamp, deepMerge, normalizeCoverUrl, normalizeUrl, safeClone } from "./utils.js";
...
const DEFAULT_LINKS = [
  { id: "link_google", title: "Google", url: "https://www.google.com", cover: "", icon: "", order: 0 },
  { id: "link_github", title: "GitHub", url: "https://github.com", cover: "", icon: "", order: 1 },
  { id: "link_youtube", title: "YouTube", url: "https://www.youtube.com", cover: "", icon: "", order: 2 },
  { id: "link_bilibili", title: "Bilibili", url: "https://www.bilibili.com", cover: "", icon: "", order: 3 }
];
...
function sanitizeUi(uiConfig) {
  ...
  result.gridColumns = clamp(Number(result.gridColumns) || 5, 2, 5);
  return result;
}
...
function sanitizeQuickLinks(list) {
  ...
  const normalized = list
    ...
    .map((item, index) => {
      ...
      const icon = normalizeCoverUrl(item.icon || "");
      return {
...
function migrateLegacyConfig(workingConfig, version) {
  ...
  if (version < 2) {
    if (!config.ui) {
      config.ui = { showClock: true, layout: "cozy", gridColumns: 5 };
    }
    if (!config.theme) {
      config.theme = {};
    }
  ...
  if (version < 4) {
    ...
    if (typeof config.widgets.weather.useLocation === "undefined") {
      config.widgets.weather.useLocation = true;
    }
    if (typeof config.widgets.weather.location === "undefined") {
      config.widgets.weather.location = null;
    }
  }
  if (version < 5) {
    if (!config.widgets) {
    ...
    if (typeof config.widgets.weather.forecastDays === "undefined") {
      config.widgets.weather.forecastDays = 3;
    }
  }
...
export function migrateConfig(rawConfig) {
  ...
  merged.widgets.weather.forecastDays = clamp(Number(merged.widgets.weather.forecastDays) || 3, 3, 6);
  merged.widgets.weather.useLocation = Boolean(merged.widgets.weather.useLocation);
  if (
    !merged.widgets.weather.location ||
    typeof merged.widgets.weather.location !== "object"
  ) {
    merged.widgets.weather.location = null;
  } else {
    const lat = Number(merged.widgets.weather.location.lat);
    const lon = Number(merged.widgets.weather.location.lon);
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
      ...
      merged.widgets.weather.location = {
        lat,
        lon,
        label: String(merged.widgets.weather.location.label || "当前位置")
      };
    }
  }
